...
for (i in 1:nrow(conditions)) {
  significant <- logical(nsim)
  for (j in 1:nsim) {
    datlist <- simulate_data(nsites_per_forest = 10,
                             nreps = 3, 
                             true_forest_effect = 1, 
                             true_forest_sd = 0.25, 
                             logit_occupancy_intercept = 0, 
                             logit_detection_intercept = logit(conditions$mean_det_prob[i]))
...
```{r}
# New simulation function -- now including an effect of forest edge dist on det
simulate_data_2 <- function(nsites_per_forest, 
                            nreps,
                            true_forest_effect, 
                            true_forest_effect_on_det, 
                            true_forest_sd,
                            logit_occupancy_intercept,
                            logit_detection_intercept) {
  ...
  p <- expit(logit_detection_intercept + true_forest_effect_on_det * siteCovs$dist_edge)
  ...
  for (i in 1:nrow(y)) {
    ...
    y[i,] <- rbinom(n = nreps, size = 1, prob = z[i] * p[i])
  }
...
# New estimation function -- now including an effect of forest edge dist on det
fit_and_evaluate_one_2 <- function(this_datlist) {
  silence <- suppressWarnings(capture.output(
    this_fit <- stan_occu(~ dist_edge ~ dist_edge + (1|forest_ID), data = this_datlist$umf,
                        iter = 4000, warmup = 2000, verbose = FALSE, refresh=-1)
  ))
...
for (i in 1:nrow(conditions)) {
  significant <- logical(nsim)
  for (j in 1:nsim) {
    datlist <- simulate_data_2(nsites_per_forest = 10,
                             nreps = 3, 
                             true_forest_effect = 1,
                             true_forest_effect_on_det = conditions$det_effect_forest[i],
                             true_forest_sd = 0.25, 
                             logit_occupancy_intercept = 0, 
                             logit_detection_intercept = 0)
    
    significant[j] <- fit_and_evaluate_one_2(datlist)
  }
  conditions$power[i] <- mean(significant)
}

# We need more simulations for this to be stable/meaningful
conditions 
```
