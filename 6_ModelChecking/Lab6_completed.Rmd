...
quantities that depend on the parameters, such as $p_i$ and $lambda_i$.

```{r}
F_dist <- numeric(nSamp)

# Simulated data
set.seed(1)
system.time({
for(i in 1:nSamp) {
    values(cmod, postNames) <- samples_long[i, ]  # assign 'flattened' values
    
    # Use 'calculate' to make sure lambda and p are updated
    cmod$calculate("p")
    cmod$calculate("lambda")

    cmod$simulate("y", includeData = TRUE)
    
    F_dist[i] <- sum(
      (sqrt(cmod$y) - sqrt(cmod$N * cmod$p))^2
    )
}
})
...
# Real data
cmod$y <- as.matrix(dat$y)
F_observed <- numeric(nSamp)
...
# Mean observed deviance
F_obs_mean <- mean(F_observed)

{
  hist(F_dist, xlim = range(c(F_dist, F_obs_mean)),
      main = "Discrepancy = Freeman-Tukey stat.", 
      xlab = "F")
  abline(v = F_obs_mean, col = 'red')
}
