...
ggplot(dat,
  ...
  xlab("Moisture") + ylab("Number of fruit") +
  theme_minimal() +
  facet_wrap(~paste0("Field ", field))

```

Within each field, there's a positive relationship between moisture and number of fruit, but this is obscured due to the variation between fields. There are some field-specific factors that apparently make growing much worse in certain fields.
...
Let's see what kind of predictions these two models make in each field:

```{r}

dat$predicted_glm <- predict(glm_fit, type = "link")

ggplot(dat,
       aes(moisture_scaled, n_fruit, 
                 col = as.factor(field)), group = field) +
  # geom_point() +
  geom_line(mapping = aes(moisture_scaled, predicted_glm, col = as.factor(field))) +
  xlab("Moisture") + ylab("Link-scale number of fruit") +
  theme_minimal()

dat$predicted_glmm <- predict(glmm_fit, type = "link")

ggplot(dat,
       aes(moisture_scaled, n_fruit, 
                 col = as.factor(field)), group = field) +
  # geom_point() +
  geom_line(mapping = aes(moisture_scaled, predicted_glmm, col = as.factor(field))) +
  xlab("Moisture") + ylab("Link-scale number of fruit") +
  theme_minimal()
...
Let's return to the beetles survey data from Lab 1. Recall that beetles were surveyed at a number of locations at different altitudes.
...
Since plot is used as an index, it's very important
that it's provided as an integer starting at 1. Conveniently, it's already
formatted that way, but if it weren't; for example, if it were provided as
some string (e.g. "Plot1", "Plot2"...), we'd need to convert it ourselves.
...
In this example, we'll use a slightly modified version of a GLMM---
one that we'll need NIMBLE to estimate.

[iNaturalist](inaturalist.org) is a participatory science repository that
collects amateur observations from around the world. Users submit photos of
organisms they see and a community of observers provided species identifications.
...
samples <- runMCMC(compiled_list$mcmc, niter = 10000, nburnin = 2500,
                   thin = 1, nchains = 3, samplesAsCodaMCMC = TRUE)
