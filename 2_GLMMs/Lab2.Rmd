...
**What if we wanted a random slope for altitude, by year?**
**What if we wanted a random slope for altitude, by plot?**
...
We want to understand how squirrel counts, per unit effort, vary according to four covariates: EVI, human population density, precipitation, and maximum daily temperature. We also want to account for two sources of nonindependence: space and time. We'll include crossed grouping random effects of grid cell and year.
...
$$\log(\mu_i) = \beta_0 + \beta_1 \times \text{Popden}_i 
+ \beta_2 \times \text{EVI}_i + \beta_3 \times \text{Temp}_i +
\beta_4 \times \text{Precip}_i + \alpha_{\text{year}(i)} + \gamma_{\text{cell}(i)}$$
and 
...
Finally, add the rest of the model estimation workflow here.
