...
samples <- runMCMC(compiled_list$mcmc, niter = 2000, nburnin = 0,
...
# Define new model code including new quadratic term

my_code <- nimbleCode({
  
  # Data likelihood
  for (i in 1:nobs) {
    log(lambda[i]) <- beta0 + beta1 * altitude[i] + beta2 * (altitude[i]^2)
    y[i] ~ dpois(lambda[i])
  }
...
# Build the model, providing an init for beta2. We already provide altitude
my_model <- nimbleModel(
  code = my_code,
  data = list(
    altitude = beetle_data$altitude_scaled,
...
for (i in 1:nrow(lambda_mtx)) {
  lambda_mtx[i, ] <- exp(samples_mtx[i, "beta0"] + 
                         samples_mtx[i, "beta1"] * beetle_abund_pred$altitude +
                         samples_mtx[i, "beta2"] * (beetle_abund_pred$altitude^2) 
                        # ^ this is the only line I modified from the preceding example
                         ) 
}
