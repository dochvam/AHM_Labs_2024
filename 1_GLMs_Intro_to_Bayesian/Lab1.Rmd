...
2. Plot some of the covariates. Scale them?

3. Explore a few models that build up in complexity. Estimate them with NIMBLE
...
Let's estimate this modeling using NIMBLE. 
...
**What do they do mechanically in the model?**

**What do they do in the MCMC algorithm?**
...
**Why don't we need to provide values for `mu` anywhere?**
...
Since this is a simple GLM, we could have more easily estimated this model with MLE using the `glm` function. We can do so now and check how closely our Bayesian and ML results match. 

```{r}

fit <- glm(beetles ~ altitude_scaled, data = beetle_data, family = poisson(link = "log"))
